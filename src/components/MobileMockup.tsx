...
export const MobileMockup: React.FC = () => {
  const { t } = useLanguage();
  const [activeScreen, setActiveScreen] = useState<'ncds' | 'pinto' | 'pos'>('ncds');
  ...
  return (
    <div className="relative mx-auto w-full max-w-[340px] sm:max-w-[370px]">
      ...
      >
        ...
        <span className="text-xs font-semibold text-emerald-300 font-mono">Flutter 3.x + Bloc</span>
      </motion.div>
      ...
      >
        ...
        <span className="text-xs font-semibold text-cyan-300 font-mono">Clean Architecture</span>
      </motion.div>
            ...
            <AnimatePresence mode="wait">
              {activeScreen === 'ncds' && (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[10px] text-zinc-400 uppercase tracking-wider font-mono">MJU Senior Project</p>
                  ...
                  <div className="rounded-xl bg-gradient-to-br from-zinc-900 to-zinc-900/90 border border-zinc-800 p-3 shadow-inner">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <div className="bg-zinc-950/60 rounded-lg p-2 border border-zinc-800">
                        <span className="text-[10px] text-zinc-400 block">{t('น้ำตาลในเลือด', 'Blood Sugar')}</span>
                        <span className="text-sm font-bold font-mono text-emerald-400">108 <span className="text-[9px] text-zinc-400 font-normal">mg/dL</span></span>
                      </div>
                      <div className="bg-zinc-950/60 rounded-lg p-2 border border-zinc-800">
                        <span className="text-[10px] text-zinc-400 block">{t('ความดันโลหิต', 'Blood Pressure')}</span>
                        <span className="text-sm font-bold font-mono text-cyan-400">122/80 <span className="text-[9px] text-zinc-400 font-normal">mmHg</span></span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <span className="text-[10px] text-zinc-400 font-mono tracking-wider">{t('ระบบคัดกรอง 4 กลุ่มโรค', '4 Targeted Diseases')}</span>
                    
                    <div className="grid grid-cols-2 gap-1.5">
                      {[
                        { title: t('เบาหวาน', 'Diabetes'), status: 'Normal', color: 'emerald' },
                        { title: t('ความดันโลหิต', 'Hypertension'), status: 'Optimal', color: 'cyan' },
                        { title: t('โรคหัวใจ', 'Heart Disease'), status: 'Safe', color: 'emerald' },
                        { title: t('โรคอ้วน', 'Obesity'), status: 'BMI 22.4', color: 'cyan' },
                      ].map((item, idx) => (
                        <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-zinc-900/60 border border-zinc-800/80">
                          <span className="text-[11px] text-zinc-300 font-medium">{item.title}</span>
              ...
              {activeScreen === 'pinto' && (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[10px] text-zinc-400 uppercase tracking-wider font-mono">Fakduay Logistics</p>
                      <h4 className="text-sm font-bold text-white flex items-center gap-1.5">
                        Pinto Rewards & Chat
                  ...
                  <div className="rounded-xl bg-gradient-to-br from-amber-950/40 via-zinc-900 to-zinc-900 border border-amber-500/30 p-3 shadow-inner">
                    ...
                    <p className="text-[11px] text-zinc-300 mb-2">
                      {t('แชทและสั่งออเดอร์ต่อเนื่องเพื่อรับส่วนลดพิเศษ', 'Keep messaging & ordering to unlock VIP vouchers!')}
                    </p>
                    ...
                    >
                      ...
                      {isStreaked ? t('เช็คอินแต้มวันนี้แล้ว!', 'Claimed Today!') : t('กดรับแต้มประจำวัน (+50 Pts)', 'Claim Daily Streak (+50 Pts)')}
                    </button>
                  </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-zinc-900/70 border border-zinc-800 flex items-center justify-between">
                    <div>
                      <span className="text-[10px] text-zinc-400">{t('คะแนน Profile API', 'Profile Loyalty Points')}</span>
                      <span className="text-sm font-bold font-mono text-cyan-400 block">1,450 PTS</span>
                    </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-zinc-900/50 border border-zinc-800 text-[11px]">
                    ...
                    <p className="text-[10px] text-zinc-400">
                      {t('โครงสร้าง Flutter ผสาน WebView เมนูร้านค้าได้อย่างไร้รอยต่อ', 'Seamless native state sync with dynamic HTML5 merchant catalog')}
                    </p>
                  </div>
                </motion.div>
              )}
              ...
              {activeScreen === 'pos' && (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[10px] text-zinc-400 uppercase tracking-wider font-mono">Retail Module</p>
                      <h4 className="text-sm font-bold text-white flex items-center gap-1.5">
                  ...
                  <div className="p-3 rounded-xl bg-gradient-to-br from-blue-950/40 to-zinc-900 border border-blue-500/30">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs text-zinc-300">{t('ยอดชำระสุทธิ', 'Total Amount')}</span>
                      <span className="text-base font-bold font-mono text-emerald-400">฿420.00</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 p-2 bg-zinc-950 rounded-lg border border-zinc-800 text-[11px]">
                      ...
                      <div>
                        <span className="font-semibold text-zinc-200 block">PromptPay QR Code</span>
                        <span className="text-[9px] text-zinc-400 font-mono">Ref: INV-256903-88</span>
                      </div>
                    </div>
                  </div>
