...
export const ExperienceTimeline: React.FC = () => {
  ...
  return (
    <section id="experience" className="py-20 border-b border-zinc-200 dark:border-[#243b30]">
      ...
      <div className="divide-y divide-zinc-200 dark:divide-[#243b30]">
        {experiences.map((exp: ExperienceItem, idx: number) => (
          ...
          >
            ...
            <div className="md:col-span-4 space-y-2">
              <span className="text-xs font-mono text-[#18231d] dark:text-[#bfdb39] font-bold block">
                {lang === "th" ? exp.periodTh : exp.periodEn}
              </span>
              <span className="inline-block text-[11px] font-mono uppercase tracking-wider text-zinc-700 dark:text-[#e9e8e8] bg-zinc-100 dark:bg-[#121e17] px-2.5 py-0.5 rounded border border-zinc-200 dark:border-[#243b30]">
                {lang === "th" ? exp.badgeTh : exp.badgeEn}
              </span>
              <p className="text-xs text-zinc-500 dark:text-[#969696] font-mono flex items-center gap-1 pt-1">
                <MapPin className="w-3.5 h-3.5 text-[#bfdb39]" />
            ...
            <div className="md:col-span-8 space-y-4">
              <div>
                <h3 className="text-xl sm:text-2xl font-bold text-[#18231d] dark:text-[#fafafa] tracking-tight group-hover:text-[#bfdb39] transition-colors">
                  {lang === "th" ? exp.roleTh : exp.roleEn}
                </h3>
                <p className="text-sm font-medium text-[#bfdb39] mt-0.5 font-mono">
                  {lang === "th" ? exp.companyTh : exp.companyEn}
                </p>
              </div>

              <p className="text-sm text-zinc-700 dark:text-[#c2c2c2] leading-relaxed">
                {lang === "th" ? exp.descriptionTh : exp.descriptionEn}
              </p>

              {/* Bullet points */}
              <ul className="space-y-2 text-xs sm:text-sm text-zinc-700 dark:text-[#c2c2c2]">
                {(lang === "th" ? exp.bulletsTh : exp.bulletsEn).map(
                  (bullet, bIdx) => (
                    <li key={bIdx} className="flex items-start gap-2.5">
                      <span className="text-[#bfdb39] font-mono mt-0.5 text-xs font-bold">
                        &gt;
                      </span>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {exp.skills.map((skill, sIdx) => (
                  <span
                    key={sIdx}
                    className="px-2.5 py-0.5 text-[11px] font-mono text-zinc-700 dark:text-[#969696] bg-zinc-100 dark:bg-[#121e17] border border-zinc-200 dark:border-[#243b30] rounded"
                  >
                    {skill}
                  </span>
                ))}
