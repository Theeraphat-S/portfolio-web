    ...
    <AnimatePresence>
      {!isFinished && (
        <motion.div
          ...
          className="fixed inset-0 z-[1000] flex flex-col items-center justify-center bg-zinc-950 text-zinc-100 overflow-hidden select-none"
        >
          {/* Subtle dot matrix background during preloader */}
          <div className="absolute inset-0 bg-[radial-gradient(circle,_#444_0.75px,_transparent_0.75px)] opacity-20 [background-size:24px_24px]" />

          {/* SVG Handwriting "hello" Animation */}
          <div className="relative flex flex-col items-center justify-center w-full max-w-[400px] px-6">
            <svg
              className="text-zinc-100 h-16 sm:h-20 md:h-24 w-auto drop-shadow-[0_0_25px_rgba(16,185,129,0.3)]"
            ...
            <motion.div
              ...
              className="mt-6 flex items-center gap-2 text-xs font-mono tracking-widest text-zinc-400 uppercase"
            >
              <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" />
              <span>Initializing Theeraphat.S</span>
            </motion.div>
          </div>

          <div className="absolute bottom-8 text-[10px] font-mono tracking-[0.3em] text-zinc-400 uppercase">
            CHIANG MAI &bull; 2026
          </div>
        </motion.div>
      )}
    </AnimatePresence>
