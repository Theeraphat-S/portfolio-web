  ...
  projects: [
    {
      ...
      problemTh: 'กระบวนการคัดกรองโรค NCDs ในชุมชนเดิมใช้กระดาษที่มีแบบสอบถามและตัวแปรคำนวณซับซ้อน มักเกิด Human Error ในการคิดคะแนนความเสี่ยง และมีอุปสรรคสำคัญคือพื้นที่ปฏิบัติงานของ อสม. มักเป็นจุดอับสัญญาณอินเทอร์เน็ต',
      problemEn: 'Traditional community NCDs screening relied on paper forms with multi-variable risk scoring, prone to human calculation errors. Moreover, village health volunteers (VHVs) frequently operate in remote areas with unstable or no internet connectivity.',
      decisionRationaleTh: 'ย้าย Business Logic ในการประเมิน Risk Scoring และ State Validation ทั้งหมดมาทำงานบน Client-side (Flutter & BLoC) เพื่อให้สามารถคำนวณคะแนนและแสดงผลประเมินความเสี่ยงได้ทันทีแบบ Real-time แม้ไม่มีสัญญาณอินเทอร์เน็ต',
      decisionRationaleEn: 'Migrated all risk scoring business logic and form state validation to client-side (Flutter & BLoC). This enables instant, zero-latency risk evaluation and continuous operation completely offline.',
      tradeOffsTh: 'ยอมแลกความซับซ้อนของ BLoC State Machines และ Domain Validation Rules บน Client ที่สูงขึ้น เพื่อรับประกัน Zero-latency, Data Integrity และการประมวลผลที่แม่นยำ 100% หน้างาน',
      tradeOffsEn: 'Accepted higher state machine and domain validation complexity on the client side in exchange for guaranteed offline reliability, instant feedback, and 100% computational integrity.',
      evidenceTh: 'จากการทดสอบภาคสนาม (Field Testing) ร่วมกับบุคลากรและ อสม. พบว่า อสม. สับสนกับค่า Lab และศัพท์แพทย์เฉพาะทาง จึง Redesign Input ให้เป็น Visual Range Slider พร้อม Color-coded Status และระบบแปลงหน่วยอัตโนมัติ',
      evidenceEn: 'Usability field tests with healthcare workers and VHVs revealed confusion around technical lab thresholds. We redesigned inputs into visual range sliders with color-coded risk bands and automatic unit conversions.',
      outcomeTh: 'ลดเวลาเฉลี่ยในการคัดกรองต่อคนลงจาก 10-15 นาที เหลือ < 3-5 นาที (ลดลงกว่า 60%), ขจัดความผิดพลาดในการคำนวณคะแนนความเสี่ยงเป็น 0%, และส่งออกรายงานสรุปผลให้ รพ.สต. ได้ทันที',
      outcomeEn: 'Reduced screening time per patient from 10-15 mins to < 3-5 mins (>60% reduction), eliminated scoring calculation errors to 0%, and enabled immediate automated medical report generation.',
      highlightsTh: [
      ...
      metrics: [
        { labelTh: 'เวลาคัดกรอง', labelEn: 'Screening Time', value: '< 3-5 นาที' },
        { labelTh: 'ความแม่นยำคะแนน', labelEn: 'Scoring Accuracy', value: '100% (Zero Error)' },
    ...
    },
    {
      ...
      problemTh: 'ต้องการเพิ่ม Daily Active Users (DAU) และ User Engagement ภายในแอป โดยผสานหน้าเว็บ WebView ที่มีอยู่เดิมเข้ากับ Native Experience โดยไม่ทำให้ประสิทธิภาพและความลื่นไหลของแอปลดลง',
      problemEn: 'Needed to boost Daily Active Users (DAU) and engagement by seamlessly bridging legacy dynamic WebView menus with native Flutter experiences without degrading performance.',
      decisionRationaleTh: 'ออกแบบ State Bridge Controller เพื่อซิงค์ข้อมูลระหว่าง Flutter Native กับ WebView และสร้างระบบ Chat Streaks Gamification เชื่อมต่อกับ Profile API',
      decisionRationaleEn: 'Engineered a State Bridge Controller to synchronize state between native Flutter and WebView, and built a gamified Chat Streaks mechanism connected with user Profile API.',
      tradeOffsTh: 'จัดการ Memory Overhead และ Lifecycle ของ Hybrid WebView เพื่อแลกกับความยืดหยุ่นในการอัปเดตเมนูโปรโมชั่นฝั่ง Server โดยไม่ต้อง Release App Store ใหม่',
      tradeOffsEn: 'Balanced hybrid WebView memory overhead against business agility, allowing instant server-side menu updates without requiring App Store release cycles.',
      evidenceTh: 'วิเคราะห์พฤติกรรมผู้ใช้พบว่า Drop-off rate สูงขึ้นเมื่อเกิดหน้าจอโหลดค้าง จึงปรับปรุง Caching Strategy และ Optimistic UI ระหว่างสลับเมนู',
      evidenceEn: 'User telemetry indicated drop-offs during slow WebView reloads, prompting implementation of proactive caching and optimistic UI state transitions.',
      outcomeTh: 'ยกระดับ User Retention และ Chat Engagement ได้ตามเป้าหมายของทีม พร้อมทั้งส่งมอบโมดูลที่เสถียรขึ้นสู่ Production ตามกำหนดเวลาของ Sprint',
      outcomeEn: 'Met user retention and streak engagement targets while delivering robust, production-ready modules within Agile sprint schedules.',
      highlightsTh: [
    ...
    },
    {
      ...
      problemTh: 'ระบบแคชเชียร์และจัดการสต็อกแบบเดิมทำงานช้า ไม่รองรับการเชื่อมต่อขัดข้องชั่วคราว ทำให้แถวคิดเงินติดขัดและเสี่ยงต่อข้อมูลสต็อกไม่ตรงกัน (Race Condition)',
      problemEn: 'Legacy POS and stock tracking suffered from checkout bottlenecks and risk of data inconsistencies (race conditions) during intermittent network disconnections.',
      decisionRationaleTh: 'ออกแบบ Client-side Cart & Order State ที่มี Optimistic Updates และระบบคิวส่ง Request ซ้ำอัตโนมัติ (Retry Mechanism with Idempotency Key) เมื่อต่อเน็ตได้',
      decisionRationaleEn: 'Implemented optimistic cart state updates with idempotent background retry queues to ensure seamless cashier operations during network hiccups.',
      tradeOffsTh: 'ยอมรับภาระการทำ Local Queue Reconciliation และ Conflict Resolution เพื่อแลกกับความเร็วในการสแกนคิดเงินหน้าเคาน์เตอร์ที่ไม่มีทางสะดุด',
      tradeOffsEn: 'Accepted local queue reconciliation overhead to ensure zero checkout friction and non-blocking cashier UX.',
      evidenceTh: 'ทดสอบ Stress Test ในสภาวะเน็ตกระตุก พบโอกาสยิง API ซ้ำ จึงเพิ่ม Client-generated Transaction UUID เพื่อป้องกัน Double-charging',
      evidenceEn: 'Network stress testing exposed potential duplicate API triggers, leading to client-generated transaction UUIDs to guarantee zero double-charging.',
      outcomeTh: 'อัตราความถูกต้องของข้อมูลธุรกรรมและสต็อกสินค้าอยู่ที่ 99.9% และลดเวลาต่อหนึ่ง Transaction ลงอย่างชัดเจน',
      outcomeEn: 'Maintained 99.9% transaction and inventory consistency while significantly reducing average checkout duration per customer.',
      highlightsTh: [
